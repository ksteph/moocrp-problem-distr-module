...
(1) Parameter is of type object. Inside can include (* marks required):
  ...
  bVerticalXAxisLabel - Boolean whether to make the labels in the x-axis vertical (default: false)
...
****Does not actually draw graph.****
Returns an object that includes a function drawGraph, for when you are ready to
draw graph. Reason for this is, because of all the defaults, some changes may be
needed before drawing the graph returns an object with the following:
  ...
  xAxisLabel - d3 pointer to the text component that holds the x axis label
...
ksm_d3CreateStackedBarGraph = function(parameters, svg, divTooltip) {
  ...
  if ((state.xRange == undefined) || (state.yRange.length < 2 || 
    ...
    if (state.xRange == undefined){
      if (bXIsOrdinal) {
        ...
        state.xRange = [];
        for (i = Math.min.apply(null,aryXRange); i <= Math.max.apply(null,aryXRange); i += 1)
          state.xRange.push(i)
      }
    }
  ...
  if (state.bVerticalXAxisLabel) {
    ...
    state.margin.axisX = (document.getElementById(state.tag+"stacked-bar-graph-long-str")
      .clientHeight)*2+state.margin.bottom;
  }
  ...
  if ((state.colorRange.length == 2) && !(isNaN(state.colorRange[0])) &&
    ...
    for (var stackKey in state.data) {
      ...
      //stack.stackData.sort(function(a,b) { return b.color - a.color; });
  ...
  graph.drawGraph = function() {
    ...
    graph.rects = graph.svgGroup.bars.selectAll("rect")
      ...
      .attr("ksm-data-tooltip",function(d) {return d.tooltip;})
    ...
    graph.rects
      .on("mouseover", function(d) {
        ...
        var parentWidth = $("#"+graph.divTooltip.node().parentNode.id).width()

        if ((left+width+30) > parentWidth) {
        //if (false) {
          left -= (width+30);
          graph.divTooltip.style("left", left+"px");
        } else {
          graph.divTooltip.style("left", left+"px");
        }
    ...
    var xLabelPos = (graph.state.width-graph.state.margin.right-graph.state.margin.axisY)/2 + graph.state.margin.axisY;

    graph.xAxisLabel = graph.svgGroup.xAxis.append("text")
      .attr("dy","2.25em")
      .attr("transform", "translate("+xLabelPos+",0)")
      .style("text-anchor","middle")
      .text("Attempts");
